...
- ~~**2.9** gate customer-facing spine sends by autonomy~~ ✅ **SHIPPED** — money reminders already route through the gated recovery engine; `bookingReminder` now resolves `ops/booking_reminder` (A=auto-send to customer · B=draft+notify owner · C=surface-only). Owner-facing alerts stay always-allowed. · ~~**2.10** per-tenant AI cost ceiling~~ ✅ **SHIPPED** (per-tenant ceiling already enforced via `checkBudget`; this session added the **global platform backstop** `ai:budget:global:{day}` + `AI_GLOBAL_DAILY_TOKEN_CAP` and made metering **fail-open on Redis error / fail-closed on a real ceiling**) · **2.2** fold refreshers into the 06:00 brief · **2.3** durable `domain_signals` mirror · quiet-hours wiring · **1.7** notification preferences.
