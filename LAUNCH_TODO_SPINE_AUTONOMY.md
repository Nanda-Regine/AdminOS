...
### ⏭ Remaining (next):
- ~~**1.4** wire spine into more workers~~ ✅ **DONE** — payment-received→`notifyTenant` ✅, expense-submit→`notifyTenant('approval.needed')` ✅, and `bookingReminder` is now autonomy-gated (see 2.9). (Standardising the older hand-rolled workers to `notify()` is cosmetic and deferred.)
...
- ~~**2.9** gate customer-facing spine sends by autonomy~~ ✅ **SHIPPED** — money reminders already route through the gated recovery engine; `bookingReminder` now resolves `ops/booking_reminder` (A=auto-send to customer · B=draft+notify owner · C=surface-only). Owner-facing alerts stay always-allowed. · ~~**2.10** per-tenant AI cost ceiling~~ ✅ **SHIPPED** (per-tenant ceiling already enforced via `checkBudget`; this session added the **global platform backstop** `ai:budget:global:{day}` + `AI_GLOBAL_DAILY_TOKEN_CAP` and made metering **fail-open on Redis error / fail-closed on a real ceiling**) · ~~**2.2** fold refreshers into the 06:00 brief~~ ✅ **SHIPPED** — `fanOutBrief` now also fans `adminos/signals.refresh` per tenant at the 05:00 wake-up, so brief + cockpit vital-signs compute together (decoupled via the existing refresh engine). · ~~**2.3** durable `domain_signals` mirror~~ ✅ **CODE SHIPPED** — migration `20260719_domain_signals.sql` (RLS via `current_tenant_id()`) + `publishSignal` now dual-writes to the durable table (best-effort, **silent no-op until the migration is applied**) + `readAllSignals` falls back to `readDurableSignal` on a Redis miss. ⚠️ **Manual step: apply `supabase/migrations/20260719_domain_signals.sql` to prod** (project `aetydnhnxmrsgqaqtofc`) to activate durability — until then it stays Redis-only, which is acceptable for launch. · ~~quiet-hours wiring~~ ✅ **SHIPPED** & ~~**1.7** notification preferences~~ ✅ **SHIPPED** — one pure, unit-tested `lib/notifications/delivery.ts` (`whatsappAllowed` = per-type opt-out **AND** quiet hours; SAST-aware, overnight wrap; +6 tests). The spine (`notifyTenant`/`notifyContact`) gates the WhatsApp mirror through it (in-app bell always writes). Owner controls live on `/dashboard/settings/autonomy` → **Notifications** (per-category WhatsApp toggles + a quiet-hours window) backed by `POST /api/settings/notifications` (merges into `tenants.settings`, no migration). **Only external item left: 1.3 Meta template approval (owner action in Meta Business Suite).**
