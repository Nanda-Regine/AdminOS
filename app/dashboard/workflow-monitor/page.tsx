      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {(Object.entries(counts) as [keyof typeof counts, number][]).map(([status, count]) => {
            ...
            const Icon = cfg.icon
            return (
              <div key={status} className="bg-[var(--surface-1)] rounded-2xl border border-[var(--border)] p-5">
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1">{cfg.label}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[var(--surface-1)] rounded-2xl border border-[var(--border)] overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-[var(--text-primary)] text-sm">Workflow Queue</h3>
              <span className="ml-auto text-xs text-[var(--text-dim)]">{queue.length} jobs</span>
            </div>
            <div className="divide-y divide-[var(--border)] max-h-96 overflow-y-auto">
                ...
                return (
                  <div key={job.id} className="flex items-center gap-3 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--text-secondary)] truncate capitalize">
              ...
              {queue.length === 0 && (
                <div className="px-5 py-8 text-center text-[var(--text-dim)] text-sm">
          ...
          <div className="bg-[var(--surface-1)] rounded-2xl border border-[var(--border)] overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-[var(--text-primary)] text-sm">Audit Log</h3>
              <span className="ml-auto text-xs text-[var(--text-dim)]">Last 30 events</span>
            </div>
            <div className="divide-y divide-[var(--border)] max-h-96 overflow-y-auto">
              {auditLogs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-5 py-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-[var(--surface-hover)] mt-2 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[var(--text-secondary)] font-mono text-xs">{log.action}</p>
