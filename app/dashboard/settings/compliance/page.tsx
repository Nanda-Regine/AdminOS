      ...
      <div className="p-6 space-y-6">
        ...
        <Card padding="none">
          <div className="p-5 border-b border-[var(--border)]">
            <h3 className="font-semibold text-[var(--text-primary)]">Personal Information Register</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--surface-2)]">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Records</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Fields Held</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Legal Basis</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Retention</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Access</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border)]">
                {DATA_REGISTER.map((row, i) => (
                  <tr key={row.category} className="hover:bg-[var(--surface-hover)] transition-colors">
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-[var(--text-primary)]">{row.category}</p>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-xs text-[var(--text-muted)] max-w-xs">{row.fields}</td>
                    <td className="px-5 py-3.5 text-xs text-[var(--text-muted)]">{row.legalBasis}</td>
                    <td className="px-5 py-3.5">
                      <Badge variant="blue">{row.retention}</Badge>
                    </td>
                    <td className="px-5 py-3.5 text-xs text-[var(--text-muted)]">{row.who}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Data subject rights */}
          <Card>
            <h3 className="font-semibold text-[var(--text-primary)] mb-4">Data Subject Rights (POPI §23–25)</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={item.right} className="flex items-start gap-3 p-3 bg-[var(--surface-2)] rounded-lg">
                  <span className="text-lg shrink-0">{item.icon}</span>
                  <div>
                    <p className="text-sm font-medium text-[var(--text-secondary)]">{item.right}</p>
                    <p className="text-xs text-[var(--text-muted)]">{item.action}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card>
            <h3 className="font-semibold text-[var(--text-primary)] mb-4">Compliance Status</h3>
            <div className="space-y-2">
              {[
                ...
                <div key={check.item} className="flex items-center gap-3 text-sm">
                  ...
                  <span className={check.done ? 'text-[var(--text-secondary)]' : 'text-[var(--text-dim)]'}>
                    {check.item}
                  </span>
                </div>
              ))}
        ...
        {/* Right to erasure */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-2">Right to Erasure — Delete Contact Data</h3>
