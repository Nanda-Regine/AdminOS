...
export default function LangaPage() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--text-dim)] mt-0.5">AI Business Mentor</p>
          </div>
        </div>
      ...
      {/* Message list */}
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-5 bg-[var(--surface-2)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full max-w-lg">
              {STARTER_PROMPTS.map(prompt => (
                <button
                  ...
                  className="text-left text-xs px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--surface-1)] hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 text-[var(--text-muted)] transition-all shadow-sm hover:shadow-md"
                >
                  {prompt}
                </button>
              ))}
            ...
            <div
              className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'user'
                  ...
                  : 'bg-[var(--surface-1)] border border-[var(--border)] text-[var(--text-secondary)] rounded-tl-sm shadow-sm'
              }`}
            >
              {msg.role === 'user' ? (
                ...
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    h1:     ({ children }) => <h1 className="text-base font-bold text-[var(--text-primary)] mb-1 mt-2">{children}</h1>,
                    h2:     ({ children }) => <h2 className="text-sm font-bold text-[var(--text-primary)] mb-1 mt-2">{children}</h2>,
                    h3:     ({ children }) => <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-1 mt-1.5">{children}</h3>,
                    ...
                    code:   ({ children, className }) => className
                      ? <pre className="bg-[var(--surface-2)] rounded-lg px-3 py-2 overflow-x-auto text-xs text-indigo-700 my-2"><code>{children}</code></pre>
                      : <code className="bg-[var(--surface-2)] px-1.5 py-0.5 rounded text-xs text-indigo-700 font-mono">{children}</code>,
                    blockquote: ({ children }) => <blockquote className="border-l-2 border-indigo-300 pl-3 italic text-[var(--text-muted)] my-2">{children}</blockquote>,
        ...
        {loading && (
          <div className="flex gap-3">
            ...
            <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm">
              <div className="flex items-center gap-1">
                {[0, 150, 300].map(delay => (
                  <span
                    key={delay}
                    className="w-2 h-2 rounded-full bg-[var(--surface-hover)] animate-bounce"
      ...
      {/* Input */}
      <div
        className="shrink-0 border-t px-4 py-3 bg-[var(--surface-1)]"
      ...
      >
        <div className="flex items-end gap-2.5 max-w-3xl mx-auto">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 resize-none rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-[var(--text-dim)] overflow-y-auto transition-shadow"
