...
export default async function CalendarPage() {
  ...
  return (
    <div>
      <TopBar title="Schedule" subtitle="Leave requests and invoices due — next 30 days" />
      <div className="p-4 md:p-6 space-y-6">

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Approved leave */}
          <Card>
            <h3 className="font-semibold text-[var(--text-primary)] mb-4">Approved Leave</h3>
            {approvedLeave.length === 0 ? (
              <p className="text-sm text-[var(--text-dim)] text-center py-6">No approved leave in the next 30 days.</p>
            ) : (
          ...
          {/* Pending leave */}
          <Card>
            <h3 className="font-semibold text-[var(--text-primary)] mb-4">Pending Approval</h3>
            {pendingLeave.length === 0 ? (
              <p className="text-sm text-[var(--text-dim)] text-center py-6">No pending leave requests.</p>
            ) : (
              <div className="space-y-2">
        ...
        {upcomingDue.length > 0 && (
          <Card>
            <h3 className="font-semibold text-[var(--text-primary)] mb-4">Invoices Due — Next 30 Days</h3>
            <div className="space-y-2">
              {upcomingDue.map((inv) => {
        ...
        <Card padding="none">
          <div className="p-5 border-b border-[var(--border)]">
            <h3 className="font-semibold text-[var(--text-primary)]">All Leave Requests</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--surface-2)]">
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Staff Member</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Dates</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Days</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Reason</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border)]">
                {leaveRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-[var(--surface-hover)]">
                    <td className="px-5 py-3 font-medium text-[var(--text-primary)]">
                      {(req.staff as { full_name: string } | null)?.full_name}
                    </td>
                    <td className="px-5 py-3 text-[var(--text-muted)]">{req.start_date} → {req.end_date}</td>
                    <td className="px-5 py-3 text-[var(--text-muted)]">{req.days}</td>
                    <td className="px-5 py-3 text-[var(--text-muted)]">{req.reason || '—'}</td>
                ...
                {leaveRequests.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-[var(--text-dim)] text-sm">
