...
| — | *Still open before conference: `supabase/migrations/20260817_business_type_extend.sql` written but not applied to production (needs a deliberate decision, see open decisions below); RESEND_API_KEY still dead (deprioritized — demo won't touch live signup/invite); safety_incidents/employment_equity_data still have no UI* |
...
5. **NEW — apply `supabase/migrations/20260817_business_type_extend.sql`?** Adds 6 missing `business_type` enum values (creative, consulting, events, cleaning, accounting, salons) via `ALTER TYPE ... ADD VALUE`. Additive and low-risk (can't be easily reversed, but can't break anything either — nothing currently reads those values). Not applied automatically: this is a production schema change, and this session doesn't have Supabase MCP access to this project to run it directly. Options: (a) apply it yourself via the Supabase dashboard SQL editor, (b) a future session applies it via the Management API + `SUPABASE_ACCESS_TOKEN` from `.env.local`, mirroring how the Phase 0 followup migrations were applied 14 Aug. Until applied, those 6 industries' onboarding content is correct but their `business_type` stores as `'other'`.
